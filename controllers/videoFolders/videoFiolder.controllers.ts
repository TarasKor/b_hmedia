...
export const getAllFolders = async () => {
  try {
    const folders = await prisma.video_Folder.findMany();
    return folders;
  } catch (error) {
    console.error(error);
    return null;
...
export const getFolderByid = async (id: string) => {
  try {
    const folder = await prisma.video_Folder.findUnique({
      where: {
        id: id,
      },
    });
...
export const getAllParents = async (id: string) => {
  try {
    ...
    async function findParents(id: string) {
      const parent = await prisma.video_Folder.findUnique({
        where: {
          id: id,
        },
        include: {
    ...
    const rootFolder = await prisma.video_Folder.findFirst({
      where: {
...
export const createFolder = async (name: string, parent_folder_id: string) => {
  try {
    ...
    const folder = await prisma.video_Folder.create({
      data: {
        name: name,
        parent_folder_id: parent_folder_id,
      },
    });
    await prisma.video_Folder.update({
      where: {
        id: parent_folder_id,
      },
      data: {
...
export const deleteFolder = async (id: string) => {
  try {
    const folder = await prisma.video_Folder.delete({
      where: {
        id: id,
      },
    });
